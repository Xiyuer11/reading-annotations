...
> Obsidian 阅读标注插件：**想法随手记，原文不改乱，之后找得到、跳得回**。

## Why
很多时候我们在 Obsidian 里读一篇长文，看到某一句突然有想法，第一反应是“先记一下”。  
可一旦把这句话直接塞进正文，文章结构很快就会被评论、疑问、联想搅在一起；不写进正文，又容易因为临时记在别处而在几天后完全找不到当时对应的是哪一段。  
更麻烦的是，编辑模式和阅读模式的交互经常不一致，来回切换会打断思路。  

这个插件就是为这个阅读瞬间设计的：你在原文上选中一句，快速写下想法，标注会独立保存在侧边列表里；之后无论回看还是检索，都能直接跳回对应位置。  
它尽量让你“当下记得快，之后找得到”，同时把原文本身保持在一个干净、可复用的状态。
